...
오늘의 학습 API는 파이프라인이 저장한 v4 기본 계획을 읽어 최대 세 개를 제공한다. v4는
최근 7일 실제 선택분과 비교해 무전개 반복을 제외하고, 설명 깊이가 있는 후보만 발견 역할로
허용한다. 로그인 관심사는 승인된 기본 계획 안의 순서만 조정하며 탈락 후보를 되살리지 않는다.
...
- 이슈별 퀴즈
- `3/3` 완료 화면
- 오늘을 제외한 최근 14일의 날짜별 `지난 이슈` 보관함과 읽기 전용 상세 화면

게스트 사용자의 관심사, 당일 계획, 진행 상태는 `localStorage`에 저장한다. 로그인 사용자는
퀴즈 완료를 DB에도 기록할 수 있다.

**현재 프론트 디자인은 사용자가 v1 승인본으로 확정했다(2026-07-22).** 초기 인수인계 시점에는
"디자인 승인본이 아님, 재설계 필요"였으나, 사용자가 현재 디자인이 마음에 든다고 판단해
`v1.0.0` 태그로 고정했다. 앞으로 디자인을 바꾸려면 이 v1을 기준선으로 두고 별도로 진행하며,
임의로 재설계하지 않는다. 기능 흐름과 API 계약은 그대로 유지한다.

배포는 사용자 확인 기준 백엔드가 Render에서 동작하며(무료 티어 콜드스타트 이슈 있음),
프론트는 별도 호스팅(Vercel 추정)이다. 실제 URL과 각 서비스의 빌드 소스 브랜치는 저장소에
정본으로 기록돼 있지 않으니, 배포 관련 작업 전 사용자에게 현재 호스팅과 자동 빌드 소스를
확인한다. 특히 로컬 커밋이 GitHub에 push되기 전에는 자동 빌드 배포에 반영되지 않는다.
...
- 최근 14일 날짜별 최대 세 가지를 다시 읽는 보관함 화면
- Playwright MVP 흐름 테스트

보관함 API는 저장된 v4 일일 계획을 우선 사용한다. 일일 계획 저장 기능 도입 전 날짜는
해당 날짜에 생성된 후보로 최대 세 가지를 재구성하며, 오늘은 보관함에서 제외한다.
...
### 하루 세 콘텐츠 선택 기준 (P0-2, v4 운영 반영 완료 2026-07-28)
...
- **v4 검증**: 최근 7일 실제 선택분을 비교하는 무전개 반복 게이트와 발견 설명 깊이
  게이트를 추가했다. 11일 174개 후보에서 29건을 선택했고(4일은 2건), 사용자 수용률
  **100%(29/29)**를 확인했다.
- **운영 반영**: `NewsAnalyzer` 뒤에서 `DailyLearningPlanner`가 날짜당 한 번 판정해
  기본 계획·전체 후보 판정을 DB에 저장한다. API는 이 승인 집합만 읽고 관심사에 따라
  내부 순서만 조정한다. 동일 날짜 재실행은 캐시된 계획을 반환해 Vertex를 재호출하지 않는다.
- **첫 운영 계획**: 2026-07-28 후보 4건 중 focus·context 2건 선택, 발견 기준 미달로
  빈자리를 유지했다. 전체 테스트 311건과 Alembic schema check를 통과했다.
...
Alembic 현재 head는 `f4b6d8e0a2c4` 하나다. 2026-07-28 운영 DB에 v4 일일 계획 테이블
3개를 추가했고 당일 계획 1개, 후보 판정 4개, 선택 항목 2개를 저장했다.
...
`scripts/prune_pipeline_data.py`가 daily_learning_plans·news·news_cluster·news_analysis·
issue_docent·disclosures를 14일 기준으로 정리하되, 사전
(`dictionary_terms.first_issue_docent_id`)과 사용자 활동이 참조하는 행은 남긴다.
일일 계획의 항목·판정은 계획과 함께 cascade 정리된다. 기본은 dry-run이고 `--apply`를
명시해야 삭제한다.
...
(완료 2026-07-28) ~~선택 기준 v4 보정 + API·파이프라인 반영~~ — 사용자 수용률
29/29, 운영 DB 마이그레이션, 첫 당일 계획 생성, 전체 테스트 311건 통과. 상세 사양은
§3과 `docs/design/17-daily-selection-criteria.md` 참조.

1. **사용자 결정 대기 항목** (물어보고 진행)
...
2. **쉬운 설명 4차 배치** (선별 pending이 다시 생기면)
...
4. **실제 공개 배포 상태 확인**
   ...
   - 공개 배포 후 모바일에서 오늘 학습→완료→지난 이슈→다시 읽기 흐름을 확인한다.
...
- 지난 이슈를 14일보다 오래 보존할지 사용자 반응을 보고 결정한다.
...
8. 선택 기준을 잇는다면 `docs/design/17-daily-selection-criteria.md`, v4 수용률 검토
   파일, 저장된 일일 계획을 먼저 읽고 `run_selection --model v4` 회귀를 확인한다.
