...
워크플로는 저장소 최상위의 `.github/workflows/news-pipeline.yml`에 구현되어 있습니다. 백엔드가
`jangdokdae-server` 하위 폴더에 있으므로 모든 실행 명령의 작업 경로도 이 폴더로 지정합니다.
초기 운영은 권장안대로 `09:07`, `15:37` KST 하루 2회이며, 수동 실행에서는 네 가지 장 운영
세션 중 하나를 선택할 수 있습니다. 이 파일도 반드시 기본 브랜치 `main`에 병합해야 예약 실행이
활성화됩니다.
