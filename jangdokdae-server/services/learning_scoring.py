"""하루 세 콘텐츠 점수 모델 — docs/design/17-daily-selection-criteria.md의 코드 번역.

입력은 운영 후보 또는 스냅샷 v2의 후보 dict 리스트다(size·source_count·frame 포함).
"""
...
def select_daily_v4(
    ...
    """v3에 무전개 반복·발견 설명 깊이 게이트를 추가한다.

    judgments는 v3 점수 필드에 아래 필드를 더 가진다.

    - topic_overlap: 최근 발행분과 같은 주제인지
    - has_new_development: 같은 주제라면 중심에 새 사실·결정·수치가 있는지
    - explains_why: 제목·훅에 원인이나 작동 구조가 명시돼 있는지

    무전개 반복은 모든 역할에서 제외한다. 발견은 ``explains_why=True``인 후보만
    허용하며, 자격 후보가 없으면 빈자리를 억지로 채우지 않는다.
    """
    # v4는 당일 후보가 부족하면 그대로 적게 반환한다. 과거 후보로 빈자리를 채우면
    # "오늘의 학습"과 최근 발행 비교의 의미가 흐려진다.
    pool = [row for row in rows if row["run_date"] == learning_date]

    def passes_repetition_gate(row: dict) -> bool:
        judged = judgments.get(row["issue_id"], {})
        return not (
            judged.get("topic_overlap") is True
            and judged.get("has_new_development") is False
        )

    eligible = [row for row in pool if passes_repetition_gate(row)]
    ...
    context = take("context_fit", eligible)
    if context is not None:
        selected.append(("context", context))
    ...
    focus = take("focus_fit", non_index) or take("focus_fit", eligible)
    if focus is not None:
        selected.append(("focus", focus))
    ...
    discovery_eligible = [
        row
        for row in eligible
        if judgments.get(row["issue_id"], {}).get("explains_why") is True
    ]
    disjoint = [
        row
    ...
    discovery = take("discovery_fit", disjoint) or take(
        "discovery_fit", discovery_eligible
    )
    if discovery is not None:
        selected.append(("discovery", discovery))
    return selected
