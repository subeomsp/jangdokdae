...
GROUNDED_DICTIONARY_PROMPT_VERSION = "bok-definition-v4"
...
async def generate_grounded_dictionary_draft(
    ...
    prompt = (
        ...
        "원문이 정부와 중앙은행처럼 여러 주체나 방식을 핵심 범위로 제시하면 일부만 "
        "남기지 않는다. 일부 예시에만 해당하는 특징을 용어 전체의 정의로 일반화하지 않는다.\n"
        "원문이 핵심 정책, 수단, 구성 요소를 여러 개 열거하면 중요한 항목을 생략하지 않는다.\n"
...
async def verify_grounded_dictionary_draft(
    ...
    prompt = (
        ...
        "supported=false다.\n"
        "원문이 여러 핵심 주체·방식·범위를 제시했는데 후보가 일부만 남기거나, 일부 예시의 "
        "특징을 전체 정의로 일반화하면 supported=false다.\n"
